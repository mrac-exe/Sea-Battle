...
# ИМПОРТИРУЕМ НЕОБОДИМЫЕ БИБЛЕОТЕКИ И ПРОРИСОВЫВАЕМ СЕТКИ
import pygame
import random
import  copy
WHITE = (255, 255, 255)
...
# ФУНКЦИЯ ОБОЗНАЧЕНИЯ СЕТКИ ПО ОСИ Х И Y
def draw_grid():
...
#ФУНКЦИЯ ОПРЕДЕЛЕНИЯ СЕТООК ИГРОКОВ
def sign_grids():
...
#ФУНКЦИЯ ОЧЕРЧИВАНИЯ РАЗРУШЕННЫХ КОРАБЛЕЙ
...
def main():
    ...
    while not game_over:# ГЛАВНЫЙ ЦИКЛ ИГРЫ
        ...
        if computer_turn: # ВЫСТРЕЛЫ КОПЬЮТЕРА
